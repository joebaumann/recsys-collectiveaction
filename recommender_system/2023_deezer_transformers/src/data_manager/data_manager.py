...
from utils import get_nr_of_unique_tracks
import pdb
...
class DataManager():
  ...
  def __init__(self, replication_folder_name, signal_planting_strategy, foldername = "resources/data", test_size=10000, min_songs_test=10, resplit=False, dim=128):
    self.foldername = foldername
    self.replication_folder_name = replication_folder_name
    self.test_size = test_size
    self.min_songs_test = min_songs_test
    self.load_playlist_track()
    self.song_embeddings_path = '%s/embeddings/%s/song_embeddings_%d.npy' % (self.foldername, self.replication_folder_name, dim)
    self.album_embeddings_path = '%s/embeddings/%s/alb_embeddings_%d.npy' % (self.foldername, self.replication_folder_name, dim)
    self.artist_embeddings_path = '%s/embeddings/%s/art_embeddings_%d.npy' % (self.foldername, self.replication_folder_name, dim)
    self.pop_embeddings_path = '%s/embeddings/%s/pop_embeddings_%d.npy' % (self.foldername, self.replication_folder_name, dim)
    self.dur_embeddings_path = '%s/embeddings/%s/dur_embeddings_%d.npy' % (self.foldername, self.replication_folder_name, dim)
    ...
    self.n_tracks = get_nr_of_unique_tracks(signal_planting_strategy)
  ...
  def load_playlist_track(self):
    self.playlist_track = load_npz("%s/rta_input/%s/playlist_track.npz" % (self.foldername, self.replication_folder_name))
  
  def load_playlist_artist(self):
    self.playlist_artist = load_npz("%s/rta_input/%s/playlist_artist.npz" % (self.foldername, self.replication_folder_name))

  def load_playlist_album(self):
    self.playlist_album = load_npz("%s/rta_input/%s/playlist_album.npz" % (self.foldername, self.replication_folder_name))

  def load_metadata(self):
    # load metadata information such as album/artist/duration/popularity

    self.song_album = np.load("%s/rta_input/%s/song_album.npy" % (self.foldername, self.replication_folder_name))
    self.song_artist = np.load("%s/rta_input/%s/song_artist.npy" % (self.foldername, self.replication_folder_name))
    ...
    with open("%s/rta_input/%s/album_ids.pkl" % (self.foldername, self.replication_folder_name), 'rb+') as f:
      self.album_ids = pickle.load(f)

    with open("%s/rta_input/%s/artist_ids.pkl" % (self.foldername, self.replication_folder_name), 'rb+') as f:
      self.artist_ids = pickle.load(f)

    with open("%s/rta_input/%s/artist_songs.pkl" % (self.foldername, self.replication_folder_name), 'rb+') as f:
      self.artist_songs = pickle.load(f)
  
    with open("%s/rta_input/%s/album_songs.pkl" % (self.foldername, self.replication_folder_name), 'rb+') as f:
      self.album_songs = pickle.load(f)

    with open("%s/rta_input/%s/artist_names.pkl" % (self.foldername, self.replication_folder_name), 'rb+') as f:
      self.artist_names = pickle.load(f)

    with open("%s/rta_input/%s/album_names.pkl" % (self.foldername, self.replication_folder_name), 'rb+') as f:
      self.album_names = pickle.load(f)

  def load_track_info(self):
    with open("%s/rta_input/%s/tracks_info.json" % (self.foldername, self.replication_folder_name)) as f :
  ...
  def split_sets(self):
    ...
    print("why am I here? splitting into train/val/test and collective should have happened in the format_rta_input.py file already as part of the preprocessing.")
    pdb.set_trace()
    ...
    np.save('%s/dataset_split/%s/train_indices' % (self.foldername, self.replication_folder_name), train_indices)
    np.save('%s/dataset_split/%s/val_indices' % (self.foldername, self.replication_folder_name), val_indices)
    np.save('%s/dataset_split/%s/test_indices' % (self.foldername, self.replication_folder_name), test_indices)

  def get_indices(self, set_name, resplit = False):
    if resplit:
      # raise error because we should not resplit the data
      raise ValueError("Resplitting the data is not allowed. The data should be split into train/val/test and collective already as part of the preprocessing.")
      self.split_sets()
    return np.load("%s/dataset_split/%s/%s_indices.npy" % (self.foldername, self.replication_folder_name, set_name))
...
class SequentialTrainDataset(Dataset):
    # This class is used to load the training set. If a playlist is shorter than max_size = 50 song, select entire playlist
